...
def format_markdown(report: Dict[str, Any]) -> str:
    ...
    if tokenizer and tokenizer.get("present"):
        ...
        if special_tokens:
            if isinstance(special_tokens, list):
                lines.append(f"- **Special Tokens**: {', '.join(str(t) for t in special_tokens[:10])}")
            else:
    ...
    if arch_details and any(v is not None for v in arch_details.values()):
        ...
        for key in ["hidden_size", "num_layers", "num_attention_heads", 
                    "intermediate_size", "max_position_embeddings", "vocab_size"]:
            value = arch_details.get(key)
            if value is not None:
                formatted_key = key.replace("_", " ").title()
                lines.append(f"- **{formatted_key}**: {value:,}" if isinstance(value, int) else f"- **{formatted_key}**: {value}")
